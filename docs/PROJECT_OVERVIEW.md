...
- **Заметки чата** — темы для разговора (`chat_notes`), модалка «Заметки чата», вставка в поле ввода («Хочу обсудить: …»)
...
chat/_hooks/
  ...
  useChatNotes.js        # Заметки чата (GET/POST /api/chat/notes)
...
| `chat_notes` | Заметки чата (тема/мысль для диалога, не цитаты из сообщений) |
