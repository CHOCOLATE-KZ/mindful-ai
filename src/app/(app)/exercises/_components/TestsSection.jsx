...
import TestsGateBanner from "./TestsGateBanner";
...
export default function TestsSection({ filter, gate, pendingRecommendations }) {
  ...
  const generatedRec = pendingRecommendations?.generated || null;

  const catalogRec = pendingRecommendations?.catalog || null;



  const catalogTest = useMemo(() => {

    if (!catalogRec?.catalogKey) return null;

    return TESTS.find((t) => t.key === catalogRec.catalogKey) || null;
  ...
  const otherTests = useMemo(() => {
    ...
    const excludeKey = catalogRec?.catalogKey || null;

    return filteredTests.filter((t) => t.key !== excludeKey);

  }, [filteredTests, catalogRec]);



  const hasPersonal = Boolean(generatedRec);

  const hasCatalog = Boolean(catalogRec && catalogTest);

  const hasAnyRecommendation = hasPersonal || hasCatalog;
  ...
  return (
    ...
    <section className="space-y-8 relative55">
      ...
      <TestsGateBanner gate={gate} showRecommendation={false} />
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between pt-4">
        ...
        <div className="max-w-xl">
          ...
          <p className="mt-3 text-base sm:text-lg text-slate-600 font-light leading-relaxed">
            ...
            ИИ создаёт персональный опросник под ваш диалог и рекомендует шкалу из каталога.

            Ниже — все доступные тесты.
      ...
      {hasPersonal && (
        ...
        <div className="space-y-4">
          ...
          <h3 className="text-lg font-bold text-slate-800">Персональный тест от ИИ</h3>

          <RecommendedTestCard recommendation={generatedRec} />
      ...
      {hasCatalog && (
        ...
        <div className="space-y-4">
          ...
          <h3 className="text-lg font-bold text-slate-800">Рекомендация из каталога</h3>

          <RecommendedTestCard recommendation={catalogRec} catalogTest={catalogTest} />
      ...
      <div className="space-y-4">
        ...
        {hasAnyRecommendation && (
          ...
          <h3 className="text-lg font-bold text-slate-800 pt-2">Все тесты</h3>
