    ...
    <>
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-black/10 dark:border-white/10 bg-white/85 dark:bg-slate-950/90 backdrop-blur-xl">
        <form onSubmit={onSend} className="mx-auto max-w-4xl px-4 py-4">
          {/* панель */}
          <div className="rounded-2xl bg-white dark:bg-slate-800 shadow-sm ring-1 ring-black/10 dark:ring-white/10 px-4 py-3">
            <div className="flex items-end gap-3">
              <textarea
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Напишите, что чувствуете…"
                className="w-full min-h-[48px] max-h-36 resize-none rounded-xl border border-black/10 dark:border-white/10 bg-white dark:bg-slate-700 dark:text-slate-100 dark:placeholder-slate-400 px-4 py-3 text-[15px] leading-6 outline-none
