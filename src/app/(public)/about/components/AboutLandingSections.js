          ...
          <ul className="mt-6 space-y-3">
            {points.map((point) => (
              <li key={point} className="flex items-start gap-3 text-slate-700">
                <CircleCheck className="mt-0.5 h-5 w-5 flex-shrink-0 text-blue-600" />
                <span>{point}</span>
              </li>
            ))}
          </ul>
  ...
  return (
    <section className="pb-6">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        ...
        <Link
          href="/psychology"
          className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-4 py-2.5 font-semibold text-slate-700 transition-colors hover:border-blue-400 hover:text-blue-700"
        >
          Смотреть всё
          <ArrowRight className="h-4 w-4" />
        </Link>
